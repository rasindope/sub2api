...
func TestUsageLogRepositoryGetAPIKeySpendingRankingIncludesIPUsage(t *testing.T) {
	db, mock := newSQLMock(t)
	repo := &usageLogRepository{sql: db}
	...
	rows := sqlmock.NewRows([]string{
		...
		"average_duration_ms", "openai_usd", "deepseek_cny", "glm_points", "qwen_credits",
		"distinct_ip_count", "ip_usages", "total_actual_cost", "total_requests", "total_tokens",
	}).AddRow(int64(9), "client-a", int64(7), "rank@example.com", 12.5, int64(4), int64(400), 1500.0, 3.5, 1.84, 6126.1, 1920.0, int64(2), `[{"ip_address":"203.0.113.8","requests":3,"first_seen_at":"2025-01-01T00:30:00.000Z","last_seen_at":"2025-01-01T01:00:00.000Z"}]`, 12.5, int64(4), int64(400))

	// 上游口径四列必须留在 SQL 里：账号识别靠 base_url，Qwen 系数 53.5 与前端展示同源。
	mock.ExpectQuery(`(?s)WITH key_spend AS \(.*api\.deepseek\.com.*bigmodel\.cn.*token-plan.*53\.5.*key_ip_rows AS \(.*BTRIM\(u\.ip_address\).*ip_rank <= 20`).
		WithArgs(start, end, 12).
		WillReturnRows(rows)
	...
	require.Equal(t, 3.5, got.Ranking[0].OpenAIUSD)
	require.Equal(t, 1.84, got.Ranking[0].DeepSeekCNY)
	require.Equal(t, 6126.1, got.Ranking[0].GLMPoints)
	require.Equal(t, 1920.0, got.Ranking[0].QwenCredits)
...
func TestUsageLogRepositoryGetAPIKeyIPActivity(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)WITH history AS \(.*created_at >= \$1::timestamptz.*created_at < \$2::timestamptz.*ORDER BY kh.requests DESC`).
		WithArgs(start, end).
		WillReturnRows(rows)
